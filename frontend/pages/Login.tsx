...
const Login = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    // If it looks like a phone number (no @), validate it's exactly 10 digits
    if (!emailOrPhone.includes('@') && emailOrPhone.length !== 10) {
    ...
    if (result.success) {
      ...
      // always go to home; Home component will show role selector if needed
      navigate('/home');
    } else {
      toast.error(result.error || 'Invalid credentials. Please sign up first.');
    }
  };
        ...
        <div className="glass rounded-2xl p-8 space-y-6">
          {!showForgot ? (
            <form onSubmit={handleLogin} className="space-y-5">
              ...
              <Button type="submit" disabled={loading} className="w-full gradient-primary text-primary-foreground h-12 text-base font-semibold rounded-xl glow-primary hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed">
